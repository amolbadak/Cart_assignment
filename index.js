const foodItem = [
  ...
  function displayItem(items)
  {
    ...
    filteredvalue=foodItem.filter(fooditem=>fooditem.category===d)    
    /*  */
    for(i=0;i<filteredvalue.length;i++)
    {
        //console.log(foodItem[i].img)
        productname=filteredvalue[i].name
        html += `<div class="row text-center m-b-5">
        <div class="col-md-3 text-center">
            <h3><img src="${filteredvalue[i].img}" width="150px" height="150px"/></h3>
        </div>
        <div class="col-md-2">
            <h3>${filteredvalue[i].name}</h3>
        </div>
        <div class="col-md-2">
        <h3>${filteredvalue[i].price}</h3>
        </div>
        <div class="col-md-2">
        <h3>${filteredvalue[i].rating}</h3>
        </div>
        <div class="col-md-2">
            <h3><input type ="checkbox" id='checkid' onClick=addToCart(${filteredvalue[i].id})></input></h3>
        </div>
        <hr>`
    }
   ...
   */
   ...
   function addToCart(itemid) { 
        ...
        var foodid=foodItem.findIndex(food=>food.id===itemid);
        if (cartData.indexOf(foodItem[foodid]) == -1) {
        cartData.push(foodItem[foodid]);       
 ...
 /* function addToCart(itemid) { 
    ...
    var itemToAdd=foodItem.find(food=>food.id===itemid);
    if (cartData.indexOf(foodItem[itemToAdd]) == -1) {
 ...
 // console.log(cartData);
  ...
  function cartItems(){
    ...
    var tbodyhtml=document.getElementById('table-body');
    tbodyhtml.innerHTML = "";
    ...
    {     
        ...
        <td><button class="decreaseitem"  onclick="decreaseitem(${cartData[i].id})">-</button>${cartData[i].quantity}
        <button class="increaseitem"  onclick="increaseitem(${cartData[i].id})">+</button></td>
        <td>${cartData[i].price}</td>
    </tr>`
    }  

    tbodyhtml.innerHTML = temphtml;
  }
  ...
  { 
    let foodid=foodItem.findIndex(food=>food.id===itemid);
    let originalPrice=foodItem[foodid].price;    
    console.log(cartData.filter(cart=>cart.id===itemid)
    .map((cart,index)=>{
      cart.quantity=cart.quantity+1;
      cart.price=cart.quantity*originalPrice;             
      return cart;
    }))
    cartItems();
    totalAmount();
  ...
  { 
    ...
    let itemInCart=foodItem.find(food=>food.id===parseInt(itemid));    
    ...
    function decreaseitem(itemid){
        ...
        let itemToDecr=foodItem.find(food=>food.id===parseInt(itemid));    
        ...
        if(itemToDecr.quantity>1)
        {
          ...
          itemToDecr.quantity = itemToDecr.quantity-1;
          itemToDecr.price = curPrice * itemToDecr.quantity;
